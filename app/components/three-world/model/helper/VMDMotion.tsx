...
function VMDMotion({ target, mixer, vmdFile, motionName, resetPose, resetPhysic }: Props) {
    ...
    const {
        ...
        delete: button(() => {
            usePresetStore.setState(({ models }) => {
                ...
                if (motionNames.indexOf(motionName) > -1) {
                    motionNames.splice(motionNames.indexOf(motionName), 1)
                }
    ...
    // Init triggered by
    useEffect(() => {
        if (!isInit || !player.paused) return
        switch (triggeredBy) {
    ...
    // Triggered by player
    useEffect(() => {
        if (!isInit || triggeredBy != TriggerMode.PLAYER) return
        const onPlay = () => {
            enableMotion(true)
        }
